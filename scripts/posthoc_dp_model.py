...
def main():
    # --- Argument parsing ---
    parser = argparse.ArgumentParser(description="Run Post-Hoc DP Autoencoder hyperparameter tuning.")
    ...
    try:
        ...
        os.makedirs("hyperparams/posthoc_dp", exist_ok=True)  # Ensure hyperparams/posthoc_dp directory exists
        with open("hyperparams/posthoc_dp/{}.pkl".format(args.version), "wb") as f:
            pickle.dump(best_params, f)

        # --- Save the best model ---
        os.makedirs("models/posthoc_dp", exist_ok=True)  # Ensure models/posthoc_dp directory exists
        best_model.save(f"models/posthoc_dp/{args.version}")
        ...
        results_path = f"results/metrics/posthoc_dp.csv"
