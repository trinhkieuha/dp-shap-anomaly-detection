...
def main():
    ...
    print(f"Arguments: metric={metric}, model_type={model_type}, continue_run={continue_run}, background_size={background_size}, nsamples={nsamples}")

    # --- Run SHAP explanations ---
    shap_init = ShapKernelExplainer(model_type=model_type, metric=metric, continue_run=continue_run)
    shap_init.explain_all_versions(background_size=background_size, nsamples=nsamples)
